...
double GenMatrix::add(int i, int j, double val){
	if (i < 0 || i >= numRow || j < 0 || j >= numColumn) {
        cerr << "Error: Invalid indices in GenMatrix::add(" << i << ", " << j << ")" << endl;
        return 1;
    }
	int mu, nu, numnz;
	double* tmpVal;
...
void GenMatrix::productGtG( GenMatrix& A, GenMatrix& B, GenMatrix& AB ){
	...
	if( B.getNumRow() != m || 
	   AB.getNumRow() != n || AB.getNumColumn() != o){
		std::cout << "Error: Invalid indices in productGtG" << std::endl;
		throw OverIdxErr();
	}
...
//    not converged : -1
	...
	// サイズチェック
    if (A.numRow != A.numColumn || A.numRow != L.numRow || L.numRow != L.numColumn) {
        std::cout << "Error: Matrix size mismatch in iccgSolv" << std::endl;
        return -1;
    }
