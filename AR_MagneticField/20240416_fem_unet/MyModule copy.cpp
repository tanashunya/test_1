...
int Carray::read_mesh(){
    ...
    {   
        ...
        nodes.push_back(std::make_unique<NODE>(node_index-1, node_x, node_y, node_z));
    }
    ...
    //     elements.push_back(std::make_unique<ELEMENT>(elemNum, matNum, *nodes[node_idx1-1], *nodes[node_idx2-1], *nodes[node_idx3-1]));
    // }
    fclose(fp);
    return 0;
}
