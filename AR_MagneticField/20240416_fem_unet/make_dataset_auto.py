...
def make_dataset_auto(max_iteration, initial_data_index, fin_data_index):
    ...
    for iter in range(max_iteration):
        ...
        base_size   = 30
        fluctuation = 3
        size        = base_size + random.uniform(-fluctuation, fluctuation) # ベースサイズにゆらぎを与える
        ...
        if num_mag_actual==0:
            ...
            make_mesh(float(config.WIDTH), float(config.HEIGHT), objects, mesh_size)            
...
if __name__=="__main__":
    make_dataset_auto(max_iteration=100000, initial_data_index=10001, fin_data_index=20000)