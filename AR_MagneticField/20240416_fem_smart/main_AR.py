...
while True:
    ...
    if ids is not None:
        ...
        for i in range(len(ids)):
            ...
            markers.append(Marker(marker_id, marker_corners, num_mag))
            if marker_id == id_magnet:
                num_mag += 1
        ...
        if config.MAKE_DATASET:
            ...
            path = "../../MyWorkspace/data/input/" + date + ".png"
        ...
        if myfunc3.is_possible_to_make_mesh(markers) is False and config.VIRTUAL:
            ...
            theta = 0.0
            my_carray = Carray.Carray(theta, len(theta_of_magnets), theta_of_magnets, int(width), int(height), config.MAKE_DATASET)
            my_carray.fem()
