...
from RBFN_model import Fm_RBFN_2, Phi_RBFN, Fm_Linear
import torch
...
time_span = 50.0  # シミュレーション時間
dt = 0.1  # 時間ステップ

global fm_model
global phi_model
# fm_model = Fm_RBFN_2('F_m_final_model_0.00000000016.pth')
fm_model = Fm_Linear('sum2.csv')
phi_model = Phi_RBFN('Phi_sum_final_model.pth')

# 電磁力 Fm と鎖交磁束 Φ の関数定義
def Fm(z1, z2, i_f, i_s):
    return fm_model.interpolate([[z1- z2, i_f, i_s]])
    # return fm_model.predict([[z2 , i_f, i_s]])
...
def f_1_2(z1, z2, z1_dot, z2_dot, i_f, i_s, t):
    ...
    # g1 = (z1_dot*(-c1 - c12) + z2_dot*(c12) + z1*(-k1 - k12) + z2*(k12) + Fm_val - m1 * x0_dotdot(t)) / m1
    # g2 = (z2_dot*(-c2 - c12) + z1_dot*(c12) + z2*(-k2 - k12) + z1*(k12) - Fm_val - m2 * x0_dotdot(t)) / m2
    g1 = (z1_dot*(-c1 - c12) + z2_dot*(c12) + z1*(-k1 - k12) + z2*(k12) - Fm_val - m1 * x0_dotdot(t)) / m1
    g2 = (z2_dot*(-c2 - c12) + z1_dot*(c12) + z2*(-k2 - k12) + z1*(k12) + Fm_val - m2 * x0_dotdot(t)) / m2
    return g1, g2
...
if __name__ == "__main__":
    ...
    plt.gca().set_xlim([t[0], t[-1]])  # x軸の範囲を設定
    plt.gca().set_ylim([min(z1_minus_z2_vals), max(z1_minus_z2_vals)])  # y軸の範囲を設定
    ...
    fm_values = []
    fm_times = np.arange(-0.020, 0.020, 0.0001)
    for t in fm_times:
        fm_values.append(Fm(t, 0, i_f, 0.0))
    ...
    plt.gca().set_xlim([fm_times[0], fm_times[-1]])  # x軸の範囲を設定
    plt.gca().set_ylim([min(fm_values), max(fm_values)])  # y軸の範囲を設定
    plt.savefig('png/fm_linear_plot.png')