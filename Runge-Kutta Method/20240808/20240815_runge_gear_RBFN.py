...
def differential_equation_2(z1, z2, if_current, is_current):
    return Phi(z1, z2, if_current, is_current)

# Gearの2次後退差分法による式(2)の数値解法（陰的）
def gear_second_order_backward_diff_step(phi_current, phi_prev, z1, z2, if_current, is_current, dt):
    def residual(phi_next):
        phi_dot = differential_equation_2(z1, z2, if_current, is_current)
        return phi_next - (4/3) * phi_current + (1/3) * phi_prev - (2/3) * dt * phi_dot

    # 初期推定値としてphi_currentを使用
    phi_next = phi_current
    tol = 1e-6
    max_iter = 100000

    for _ in range(max_iter):
        res = residual(phi_next)
        if np.abs(res) < tol:
            break
        # ニュートン法の更新
        phi_dot = differential_equation_2(z1, z2, if_current, is_current)
        jacobian = 1  # ここでは簡単のためにヤコビ行列を1と仮定
        phi_next -= res / jacobian
...
if __name__ == "__main__":
    ...
    for i in range(num_steps):
        ...
        phi_next = gear_second_order_backward_diff_step(phi_current, phi_prev, z1, z2, if_current, is_current, dt)
        # 更新
